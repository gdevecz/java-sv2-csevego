...
public class GuessTheNumber {
    ...
    public void startTheGame() {
        ...
        for (int numberOfGuesses = 1; numberOfGuesses <= maxNumberOfGuesses; numberOfGuesses++) {
            ...
            if (checkTheGuess(guess, numberOfGuesses)) {
                return;
            }
        }
    ...
    //    Itt elgondolkodtam, hogy nem lenne-e szebb kiszervezni metódusba az aritmetikai kiértékelést,
    //    és azt adni tovább az io-nak, de akkor gykorlatilag nem csinálna semmit csak meghívna egy új metódust :D,
    //    lehet inkább a metódusnévre lehetne kitalálni vmi jobbat.
    private boolean checkTheGuess(int guess, int numberOfGuesses) {
        int difference = guess - theNumber;
        if (difference == 0) {
            gameIO.informAboutSuccess(theNumber, numberOfGuesses);
            return true;
        }
